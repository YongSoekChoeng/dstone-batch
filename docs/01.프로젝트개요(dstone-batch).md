...
모든 배치 잡은 **`BaseJobConfig` 클래스를 상속**하고, 클래스에 **`@AutoRegJob` 어노테이션을 선언**하여 정의합니다.

-   **`@AutoRegJob(name = "sampleJob")`**:
    -   이 어노테이션은 해당 클래스가 배치 잡임을 나타냅니다.
...
-   **`configJob()` 메서드**:
    -   `addStep()`, `addFlow()`, `addTasklet()` 등의 헬퍼 메서드를 호출하여 잡의 실행 단위를 순서대로 추가합니다.
...
2.  **`env.properties` 로드**: `setSysProperties()` 메서드가 `conf/env.properties` 파일의 내용을 시스템 프로퍼티로 로드합니다.
...
-   **Spring Batch**:
    -   `initialize-schema: NEVER`: 애플리케이션 실행 시 배치 관련 테이블을 자동으로 생성하지 않습니다. 필요시 `resources/schema/*.sql` 스크립트로 수동 생성해야 합니다.
    -   `job.enabled: true`: 잡 실행을 활성화합니다.
...
-   Jasypt 암호화 키(`jasypt.encryptor.password`)와 같이 민감하거나 환경에 따라 달라지는 설정을 정의합니다.
-   이 파일의 내용은 애플리케이션 시작 시 시스템 프로퍼티로 등록되어 `.yml` 파일 등에서 `${...}` 구문으로 참조할 수 있습니다.
...
2.  Data Flow UI 또는 Shell에서 등록된 태스크 정의를 사용하여 태스크(잡)를 실행합니다.
> 이 방식은 잡의 스케줄링, 모니터링, 중앙 관리를 용이하게 합니다.
